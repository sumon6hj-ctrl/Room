...
# ==================== REGION-BASED URL SYSTEM ====================
def get_base_url(region: str) -> str:
    """Return the base URL (without /GetLoginData etc.) for the given region."""
    region = region.upper().strip()
    if region in ('IND', 'IN'):
        return 'https://client.ind.freefiremobile.com'
    elif region in ('US', 'BR', 'NA', 'SAC'):
        return 'https://client.us.freefiremobile.com'
    else:
        # all others (BD, ME, PK, TH, TW, SG, VN, ID, RU, EU, etc.)
        return 'https://clientbp.ggpolarbear.com'

# ==================== AUTO SPAM CHECKER (UID STATUS MONITOR) ====================
_ID = '4569575037'
_PW = 'sumon_V0877_BY_SPIDEERIO_GAMING_LAHKS'
_TTL = 6 * 60 * 60
_cx = {}
...
class AutoSpamWatcher:
    ...
    def stop(self):
        self.running = False
        if self.currently_spamming: self._stop_spam()
        ...
        while self.running:
            try:
                ...
                if should_spam_target(status):
                    if not self.currently_spamming: self._start_spam()
                else:
                    if self.currently_spamming: self._stop_spam()
            except Exception as e:
            ...
            with active_spam_lock:
                if target_id not in active_spam_targets:
                    active_spam_targets[target_id] = {
                        'active': True, 'start_time': datetime.now(),
                        'duration': duration_minutes, 'auto_mode': False
                    }
                    threading.Thread(target=spam_worker, args=(target_id, duration_minutes), daemon=True).start()
                    msg = f"The spam was started on the user: {target_id}"
                    if duration_minutes: msg += f" for {duration_minutes} minutes"
        ...
        try:
            ...
            with active_spam_lock:
                if target_id in active_spam_targets:
                    ...
                    return {"status": "success", "message": f"Spam has been disabled for the user: {target_id}"}
        ...
        try:
            with active_spam_lock:
                ...
                for tid in active_targets:
                    info = active_spam_targets[tid]
                    mode = "🔁 AUTO" if info.get('auto_mode') else "🎮 MANUAL"
                    dur = ""
                    if info.get('duration'):
                        elapsed = datetime.now() - info['start_time']
                        remaining = info['duration'] * 60 - elapsed.total_seconds()
                        if remaining > 0: dur = f" ({int(remaining/60)} min left)"
                    active_targets_info.append(f"{tid}{dur} [{mode}]")
            with auto_spam_lock:
...
def start_spam():
    ...
    if not target_id: return jsonify({"status": "error", "message": "Please enter user_id"})
    return jsonify(api.process_spam_command(target_id, duration))

@app.route('/stop', methods=['GET'])
def stop_spam():
    target_id = request.args.get('user_id')
    if not target_id: return jsonify({"status": "error", "message": "Please enter user_id"})
    return jsonify(api.process_stop_command(target_id))
...
def start_auto_spam():
    ...
    return jsonify(api.process_auto_spam_command(target_id, interval))

@app.route('/auto/stop', methods=['GET'])
def stop_auto_spam():
    target_id = request.args.get('user_id')
    if not target_id: return jsonify({"status": "error", "message": "Please enter user_id"})
    return jsonify(api.process_stop_auto_spam_command(target_id))

@app.route('/check', methods=['GET'])
def check_user():
    target_id = request.args.get('user_id')
    if not target_id: return jsonify({"status": "error", "message": "Please enter user_id"})
    return jsonify(api.check_status(target_id))
...
def get_accounts():
    try:
        ...
        return jsonify({"status": "success", "data": {"connected_accounts_count": accounts_count, "connected_accounts": accounts_list}})
    except Exception as e:
...
def home():
    ...
    <h3>API Endpoints:</h3>
    <ul>
        <li><b>GET /spam?user_id=ID&duration=MIN</b> - Start manual spam (duration optional)</li>
        <li><b>GET /stop?user_id=ID</b> - Stop spam</li>
        <li><b>GET /auto?user_id=ID&interval=SEC</b> - Start auto spam (checks every SEC seconds)</li>
        <li><b>GET /auto/stop?user_id=ID</b> - Stop auto spam</li>
        <li><b>GET /check?user_id=ID</b> - Check user status</li>
        <li><b>GET /status</b> - System status</li>
        <li><b>GET /accounts</b> - Connected accounts</li>
    </ul>
    """
        ...
        with open(filename, "r", encoding="utf-8") as file:
            for line in file:
                ...
                if line and not line.startswith("#"):
                    ...
                    region = parts[2].strip() if len(parts) > 2 else 'OTHERS'
                    accounts.append({'id': uid, 'password': pwd, 'region': region})
...
class FF_CLient():
    def __init__(self, id, password, region='OTHERS'):
        self.id = id
    ...
    def Connect_SerVer(self, Token, tok, host, port, key, iv, host2, port2):
        ...
        with connected_clients_lock:
            ...
            print(f"✅ Account registered {self.id} (region: {self.region}) - Total: {len(connected_clients)}")
        while True:
            try:
                self.DaTa = self.CliEnts.recv(1024)
    ...
    def Guest_GeneRaTe(self, uid, password):
        ...
        try:
            ...
            print(f'🔑 Account {uid} – Access token obtained')
            return self.ToKen_GeneRaTe(self.Access_ToKen, self.Access_Uid)
        except Exception as e:
            print(f"❌ Guest_GeneRaTe error for {uid}: {e}")
            time.sleep(3)
            return self.Guest_GeneRaTe(uid, password)

    def GeT_LoGin_PorTs(self, JwT_ToKen, PayLoad):
        base = get_base_url(self.region)
        self.UrL = f'{base}/GetLoginData'
        self.HeadErs = {
            'Expect': '100-continue',
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                print(f"[{self.region}] GetLoginData attempt {attempt} → status {self.Res.status_code}")
                if self.Res.status_code != 200:
                    print(f"    Response preview: {self.Res.text[:200]}")
                    time.sleep(2)
                    continue
                ...
                port = address[-5:]
                port2 = address2[-5:]
                print(f"    ✅ Ports: {ip}:{port} / {ip2}:{port2}")
                return ip, port, ip2, port2
            except Exception as e:
                print(f"    ⚠️ GetLoginData error: {e}")
                time.sleep(2)
        print(f"❌ Failed to get ports for {self.id} after {max_retries} attempts")
        return None, None, None, None

    def ToKen_GeneRaTe(self, Access_ToKen, Access_Uid):
        base = get_base_url(self.region)
        self.UrL = f"https://loginbp.ggpolarbear.com/MajorLogin"
        self.HeadErs = {
            'X-Unity-Version': '2022.3.47f1',
            'ReleaseVersion': 'OB53',
            'Content-Type': 'application/x-www-form-urlencoded',
        ...
        try:
            ...
            if not all(c in '0123456789abcdefABCDEF' for c in encoded_data):
                print("⚠️ Invalid hex from EnC_AEs, using raw payload")
                self.PaYload = self.dT
            else:
                self.PaYload = bytes.fromhex(encoded_data)
        except Exception as e:
            print(f"⚠️ Encryption error: {e}, using raw payload")
            self.PaYload = self.dT
        ...
        for attempt in range(1, max_attempts+1):
            try:
                ...
                if self.ResPonse.status_code == 200 and len(self.ResPonse.text) > 10:
                    ...
                    if all([ip, port, ip2, port2]):
                        return self.JwT_ToKen, self.key, self.iv, self.combined_timestamp, ip, port, ip2, port2
                    else:
                        print(f"   GetLoginData returned incomplete ports, retrying...")
                else:
                    print(f"   MajorLogin attempt {attempt} failed: {self.ResPonse.status_code}")
                time.sleep(2)
            except Exception as e:
                print(f"   MajorLogin error: {e}")
                time.sleep(2)
        print(f"❌ Failed to generate token for {self.id}")
    ...
    def Get_FiNal_ToKen_0115(self):
        ...
        for retry in range(max_retries):
            try:
                ...
                if not result:
                    print(f"⚠️ No result from Guest_GeneRaTe, retry {retry+1}/{max_retries}")
                    time.sleep(5)
                    continue

                token, key, iv, Timestamp, ip, port, ip2, port2 = result
                if not all([ip, port, ip2, port2]):
                    print(f"⚠️ Missing ports, retry {retry+1}/{max_retries}")
                    time.sleep(5)
                    continue
                ...
                try:
                    ...
                    print(f'✅ Proxied UID: {self.AccounT_Uid}')
                except Exception as e:
                ...
                try:
                    ...
                    print(f"❌ Final token assembly error: {e}")
                    time.sleep(5)
                    continue
                ...
                print(f"❌ Get_FiNal_ToKen_0115 error (retry {retry+1}): {e}")
                time.sleep(5)

        print(f"❌ Permanent failure for account {self.id}")
        raise Exception(f"Could not initialize account {self.id}")

# ==================== MAIN ENTRY POINT ====================
def start_account(account):
    try:
        print(f"🚀 Starting account: {account['id']} (region: {account.get('region','OTHERS')})")
        FF_CLient(account['id'], account['password'], account.get('region', 'OTHERS'))
    except Exception as e:
...
def StarT_SerVer():
    print("=" * 50)
    print("   🤖 FREE FIRE AUTO SPAM BOT (OPTIMIZED)")
    print("=" * 50)
    print(f"📦 Loaded {len(ACCOUNTS)} accounts")
    print("🌐 Starting API on http://127.0.0.1:5000/")
    print("=" * 50)
    # Start API in a daemon thread
    api_thread = api_thread = threading.Thread(
    ...
    for account in ACCOUNTS:
        ...
        thread.start()
        time.sleep(1)  # reduced delay between account starts
